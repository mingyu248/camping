--------------------------------------------------------------------------------------------------------
-- ProcedureName     : ADMIN_REVIEW_ACT
-- Description       : 후기 활성화(관리자)
-- Author            : KSW, 2024-11-22
--------------------------------------------------------------------------------------------------------
CREATE OR REPLACE PROCEDURE ADMIN_REVIEW_ACT(
...
BEGIN
    ...
    END IF;
        ...
        INSERT INTO TReviewHis(
                        ...
                        VALUES(
                               pi_intReviewNo, SYSDATE, 100, '정지 사유 없음', pi_strAdminId
                               );
        po_intRetVal := 0;
