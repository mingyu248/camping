...
public class UserReviewController {
    ...
    public String myReview(Model model, HttpSession session, @RequestParam(defaultValue = "1",  name = "page")  int intPage
                                                            ,@RequestParam(defaultValue = "10", name = "limit") int intLimit) {
        
        ResultInfo          objResultInfo   = new ResultInfo();
        List<ReviewVo>      getReviewList   = new ArrayList<>();
        ...
        if (this.getLoginInfo(session).getStateCode() == 0) {
            ...
            reviewPagingMap.put("userIdOrEmail", this.getLoginInfo(session).getUserKey());
            
            getReviewList = userReviewService.getMyReviewList(reviewPagingMap);
            
            model.addAttribute("myReviewList", getReviewList);
